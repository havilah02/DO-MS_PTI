init <- function() {
  ...
  type <- 'plot'
  box_title <- 'Cumulative Peptide Identification Confidence Across Experiments (DART-ID Integration)'
  help_text <- 'The plot shows the cumulative peptide identification confidence across experiments, integrating DART-ID for improved identification accuracy.'
  source_file <- 'evidence'
    ...
    .plotdata <- function(data, input) {
      ...
      #adjust the bins to match 'peps' length
      # better for handling edge cases
      plotdata <- plotdata %>%
        dplyr::mutate(bin=findInterval(PEP, 10**peps, all.inside = TRUE)) %>%
        dplyr::group_by(Raw.file, bin) %>%
        dplyr::summarise(n=dplyr::n()) %>%
        dplyr::mutate(cy=cumsum(n),
                      pep=10**peps[pmin(bin, length(peps))]) # pmin finds the minimum value for each subscript from a set of vectors
